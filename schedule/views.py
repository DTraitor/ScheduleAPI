...
def search_groups(request):
    ...
    groups = Group.objects.filter(name__contains=name) | Group.objects.filter(short_name__contains=name)
...
def add_lesson(request):
    ...
    try:
        start_time: date = date.fromisoformat(time_start)
    except ValueError:
        return Response({"message": "Invalid start date format"}, status=400)

    try:
        end_time: date = date.fromisoformat(time_end)
    except ValueError:
        return Response({"message": "Invalid end date format"}, status=400)
...
def get_lessons_at_date(request):
    ...
    for lesson in group.lessons.filter(
        ...
        delta: timedelta = current_date - lesson.start_time
